/*
 * arch/arm/plat-s5p4418/jf4418/lcds.c
 *
...
int jf4418_setup_lcd_by_id(int id)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(jf4418_lcd_config); i++) {
		if (jf4418_lcd_config[i].id == id) {
			lcd_idx = i;
			break;
		}
	}

	if (i >= ARRAY_SIZE(jf4418_lcd_config)) {
		/* NOT found */
		return -19;
	}

	return jf4418_lcd_config[i].id;
}

int jf4418_setup_lcd_by_name(char *str)
{
	char *delim;
	int i;
	...
	if (!strncasecmp("HDMI", str, 4)) {
		struct hdmi_config *cfg = &jf4418_hdmi_config[0];
		struct nxp_lcd *lcd;

		lcd_idx = ARRAY_SIZE(jf4418_lcd_config) - 1;
		lcd = jf4418_lcd_config[lcd_idx].lcd;

		for (i = 0; i < ARRAY_SIZE(jf4418_hdmi_config); i++, cfg++) {
			if (!strcasecmp(cfg->name, str)) {
				lcd->width = cfg->width;
				lcd->height = cfg->height;
				jf4418_lcd_config[lcd_idx].name = cfg->name;
				goto __ret;
			}
		}
	}
	...
	for (i = 0; i < ARRAY_SIZE(jf4418_lcd_config); i++) {
		if (!strcasecmp(jf4418_lcd_config[i].name, str)) {
			lcd_idx = i;
			break;
		}
	}
...
struct nxp_lcd *jf4418_get_lcd(void)
{
	return jf4418_lcd_config[lcd_idx].lcd;
}

const char *jf4418_get_lcd_name(void)
{
	return jf4418_lcd_config[lcd_idx].name;
}

int jf4418_get_lcd_density(void)
{
	return jf4418_lcd_config[lcd_idx].dpi;
}
